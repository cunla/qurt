...
public class TestClusterConnector {
    private static SystemSettings systemSettings = new SystemSettings("10.76.16.150", "admin", "admin");
//    private static ClusterSettings clusterSettings = new ClusterSettings("10.64.125.226", "admin", "admin", "Israel");
    ...
    public void testClusterGetVms() {
        ...
        for (ConsistencyGroupSettings groupSettings : groupSettingsList) {
            List<ConsistencyGroupCopyUID> productionCopies = groupSettings.getProductionCopiesUID();
            ...
            for (ConsistencyGroupCopyUID productionCopy : productionCopies) {
                ...
                long clusterId = productionCopy.getGlobalCopyUID().getClusterUID().getId();
                System.out.println(String.format("Found copy %d in cluster %d", copyId, clusterId));
            }
            ...
            for (VmReplicationSetSettings replicationSetSettings : vmReplicationSetSettingsList) {
                List<VmReplicationSettings> vms = replicationSetSettings.getReplicatedVMs();
                for (VmReplicationSettings vm : vms) {
                    String vmId = vm.getVmUID().getUuid();
                    String vCenterId = vm.getVmUID().getVirtualCenterUID().getUuid();
                    int copyId = vm.getGroupCopyUID().getGlobalCopyUID().getCopyUID();
                    long clusterId = vm.getGroupCopyUID().getGlobalCopyUID().getClusterUID().getId();
                    System.out.println(String.format("VM %s in vCenter %s is under copy %d in cluster %d", vmId, vCenterId, copyId, clusterId));
                }
            }
        }
    }
    ...
    public void testgetClusterVirtualInfrastructuresStateSet() {
        ...
        for (ClusterVirtualInfrastructuresState state : stateSet.getInnerSet()) {
            ...
            for (VmState vmState : vmStateList) {
                ...
                System.out.println(String.format("VM %s in cluster %d has name %s", vmId, clusterId, vmName));
            }
        }
    }
